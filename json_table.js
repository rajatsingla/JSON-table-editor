...
JsonTableView.prototype = {
    ...
    insert: function () {
        ...
        container.insertAdjacentHTML( 'afterbegin', this._formatButtons());
    ...
    _formatButtons: function () {
        var html = '<div class="json-table-format-btn">'
        for (var i = 0; i < this.formatButtons.length; i++) {
            html += '<button data-code="5" data-formatkey="' + this.formatButtons[i] + '">' + this.formatButtons[i] + '</button>';
        }
        html += '</div>'
        return html;
    },
...
JsonTableController.prototype = {
    ...
    _bindEventOnBtn: function () {
        var self = this;
        var btn = JsonTable.qsa("button", this.view.container);
        for (var i = 0; i < btn.length; i++) {
            JsonTable.on(btn[i], "click", function(e){
...
JsonTable.prototype = {
    init: function (selector, data, options) {
        ...
        this.formatButtons = options.formatButtons || ['bold', 'italic'];
    ...
    _initTable: function (row, column) {
        this.view.insert();
        this.view.update(this.model);
